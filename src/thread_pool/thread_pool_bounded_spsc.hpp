...
bool ThreadPoolBoundedSPSC::Init(int thread_count, std::vector<int>& cpu_cores) {
    stop_ = false;
    ...
    for (int i = 0; i < thread_count; ++i) {
        ...
        thread_ptr->thread_ = std::thread(std::bind(&ThreadPoolBoundedSPSC::ThreadRun, this, i));
